...
export default function FormService() {
  ...
  const onFinish = (values: any) => {
    try {
      const formattedDateTime = dayjs(values.DatePicker).format('HH:mm DD/MM/YYYY');
  ...
  return (
    <div className='form-services'>
      <Title style={{ fontFamily: '"Playfair Display", serif' }}>QUICK RESERVATION</Title>
          ...
          <Col span={12}>
            <Form.Item name='checkbox' valuePropName='checked' label='Menu'>
              {ServicesList.map((item) => (
                <Checkbox key={item.title} value={item.title} className='menu_item'>
                  {item.title}
                </Checkbox>
              ))}
            </Form.Item>
